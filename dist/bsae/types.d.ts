 ...
 * p.weight = 170 // number
 */
declare type Merge<T, S> = T extends Date | Error | RegExp | Dayjs ? T : T extends Function ? S : T extends Array<infer TR> ? S extends Array<infer SR> ? Array<Merge<TR, SR>> : S extends ReadonlyArray<infer SR> ? ReadonlyArray<Merge<TR, SR>> : S : T extends Map<infer TK, infer TV> ? S extends Map<infer SK, infer SV> ? Map<SK, Merge<TV, SV>> : S extends ReadonlyMap<infer SK, infer SV> ? ReadonlyMap<SK, Merge<TV, SV>> : S extends WeakMap<infer SK, infer SV> ? WeakMap<SK, Merge<TV, SV>> : S : T extends Set<infer TR> ? S extends Set<infer SR> ? Set<Merge<TR, SR>> : S extends ReadonlySet<infer SR> ? ReadonlySet<Merge<TR, SR>> : S extends WeakSet<infer SR> ? WeakSet<Merge<TR, SR>> : S : T extends Record<any, any> ? {
